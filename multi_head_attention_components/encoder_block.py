...
class EncoderBlock(nn.Module):
    def __init__(self, self_attention_block : MultiHeadAttentionNetwork, feed_forward_block : FeedForwardNetwork, dropout : float, d_model:int, hyper_n:int=4, use_hyper_connection:bool=True, layer_idx:int=0, device:str='cuda'):
        super().__init__()
        ...
        if use_hyper_connection:
            self.hc_attention = ManifoldConstrainedHyperConnection(d_model, hyper_n, layer_idx*2, dropout, dynamic=True, device=device)
            self.hc_ffn = ManifoldConstrainedHyperConnection(d_model, hyper_n, layer_idx*2+1, dropout, dynamic=True, device=device)
        else:
