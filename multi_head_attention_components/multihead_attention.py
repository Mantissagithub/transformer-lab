...
class MultiHeadAttentionNetwork(nn.Module):
    ...
    def attention(query, key, value, mask, dropout : nn.Dropout):
        ...
        if mask is not None:
            # mask shape should be broadcastable to attention_scores shape
            # encoder mask: (batch, 1, 1, src_seq_len) -> broadcasts to (batch, h, tgt_seq_len, src_seq_len)
            # decoder mask: (batch, 1, tgt_seq_len, tgt_seq_len) -> broadcasts to (batch, h, tgt_seq_len, tgt_seq_len)
